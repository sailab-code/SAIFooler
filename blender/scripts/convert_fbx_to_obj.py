...
if len(argv) < 3:
    ...
    with open(mat_def_path) as mat_def_file:
        ...
        # For each row, use the material name as key and the texture name as value
        for row in csv_reader:
            material_texture_table[row[0]] = row[1]
    if len(material_texture_table) <= 0:
        print("Error: the material texture table derived from " + mat_def_path + " is empty")
    else:
            ...
            for mat_slot in imported_obj.material_slots:
                if mat_slot.material:
                    ...
                    if imported_obj_current_mat.name in material_texture_table:
                        ...
                        # Get the absolute path of the texture from the table (we assume the texture is in the same folder as the .csv file)
                        imported_object_current_mat_tex_path = os.path.split(mat_def_path)[0] + "/" + material_texture_table[imported_obj_current_mat.name]
                        # Create Image Texture node and load the albedo texture
                        imported_obj_current_mat_tex = nodes.new("ShaderNodeTexImage")
                        imported_obj_current_mat_tex.image = bpy.data.images.load(imported_object_current_mat_tex_path)
                        imported_obj_current_mat_tex.image.colorspace_settings.name = "sRGB"
                        # Generate a link between the texture color output and the material output color input
                        links.new(imported_obj_current_mat_tex.outputs["Color"], imported_obj_current_mat_shader.inputs["Base Color"])
        # Export to obj
